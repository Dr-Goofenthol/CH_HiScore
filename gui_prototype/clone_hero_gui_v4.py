"""
Clone Hero High Score Client - GUI Version V4

Enhanced UI with Activity Log + User Stats two-column layout.
"""
...
class ScoreTrackerGUI(ctk.CTk):
    ...
    def create_widgets(self):
        """Create enhanced UI with four-column layout"""
        ...
        header_frame = ctk.CTkFrame(self.main_container, border_width=2, border_color="#22b14c")
        ...
        song_container = ctk.CTkFrame(self.main_container, fg_color="transparent")
        song_container.pack(fill="both", pady=(0, 10))

        # Configure grid for equal width columns
        song_container.grid_rowconfigure(0, weight=1)
        song_container.grid_columnconfigure(0, weight=1, uniform="song")
        song_container.grid_columnconfigure(1, weight=1, uniform="song")

        # Current Song (Left Column)
        current_song_frame = ctk.CTkFrame(song_container, border_width=2, border_color="#ed1c24")
        current_song_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        ...
        self.current_song_title.pack(anchor="w", padx=12, pady=2)
        ...
        self.current_song_artist.pack(anchor="w", padx=12, pady=1)
        ...
        self.current_song_details.pack(anchor="w", padx=12, pady=1)
        ...
        self.current_song_score.pack(anchor="w", padx=12, pady=(4, 10))

        # Previous Song (Right Column)
        previous_song_frame = ctk.CTkFrame(song_container, border_width=2, border_color="#f2d800")
        previous_song_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0))
        ...
        self.previous_song_title.pack(anchor="w", padx=12, pady=2)
        ...
        self.previous_song_artist.pack(anchor="w", padx=12, pady=1)
        ...
        self.previous_song_details.pack(anchor="w", padx=12, pady=1)
        ...
        self.previous_song_score.pack(anchor="w", padx=12, pady=(4, 10))

        # ==================== TWO-COLUMN ACTIVITY LOG + USER STATS ====================
        bottom_container = ctk.CTkFrame(self.main_container, fg_color="transparent")
        bottom_container.pack(fill="both", expand=True, pady=(0, 10))

        # Configure grid for equal width columns
        bottom_container.grid_rowconfigure(0, weight=1)
        bottom_container.grid_columnconfigure(0, weight=1, uniform="bottom")
        bottom_container.grid_columnconfigure(1, weight=1, uniform="bottom")

        # Activity Log (Left Column)
        log_frame = ctk.CTkFrame(bottom_container, border_width=2, border_color="#4a9eff")
        ...
        stats_frame = ctk.CTkFrame(bottom_container, border_width=2, border_color="#ff8c00")
        stats_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0))

        stats_header = ctk.CTkFrame(stats_frame, fg_color="transparent")
        stats_header.pack(fill="x", padx=10, pady=(8, 5))
        ...
        self.refresh_stats_btn = ctk.CTkButton(
            ...
            width=30,
            height=24,
            command=self.refresh_user_stats,
        ...
        stats_content.pack(fill="both", expand=True, padx=12, pady=(0, 10))

        # Grid layout for stats
        self.stats_total_scores = ctk.CTkLabel(
            stats_content,
            text="Total Scores\n--",
            font=ctk.CTkFont(size=10),
            justify="center"
        )
        self.stats_total_scores.grid(row=0, column=0, padx=5, pady=5, sticky="ew")

        self.stats_records_held = ctk.CTkLabel(
            stats_content,
            text="Records Held\n--",
            font=ctk.CTkFont(size=10),
            justify="center"
        )
        self.stats_records_held.grid(row=0, column=1, padx=5, pady=5, sticky="ew")

        self.stats_records_broken = ctk.CTkLabel(
            stats_content,
            text="Records Broken\n--",
            font=ctk.CTkFont(size=10),
            justify="center"
        )
        self.stats_records_broken.grid(row=0, column=2, padx=5, pady=5, sticky="ew")

        self.stats_total_points = ctk.CTkLabel(
            stats_content,
            text="Total Points\n--",
            font=ctk.CTkFont(size=10),
            justify="center"
        )
        self.stats_total_points.grid(row=1, column=0, padx=5, pady=5, sticky="ew")

        self.stats_avg_accuracy = ctk.CTkLabel(
            stats_content,
            text="Avg Accuracy\n--",
            font=ctk.CTkFont(size=10),
            justify="center"
        )
        self.stats_avg_accuracy.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

        self.stats_avg_stars = ctk.CTkLabel(
            stats_content,
            text="Avg Stars\n--",
            font=ctk.CTkFont(size=10),
            justify="center"
        )
        self.stats_avg_stars.grid(row=1, column=2, padx=5, pady=5, sticky="ew")
        ...
        # Last updated label
        self.stats_last_updated_label = ctk.CTkLabel(
            stats_content,
            text="Not loaded",
            font=ctk.CTkFont(size=9),
            text_color="gray"
        )
        self.stats_last_updated_label.grid(row=2, column=0, columnspan=3, pady=(10, 5))
    ...
    def update_user_stats_display(self, stats: Optional[Dict] = None):
        """Update user stats display"""
        if not stats:
            stats = self.user_stats

        if not stats:
            # Show "Not paired" or "Loading..."
            self.stats_total_scores.configure(text="Total Scores\n--")
            self.stats_records_held.configure(text="Records Held\n--")
            self.stats_records_broken.configure(text="Records Broken\n--")
            self.stats_total_points.configure(text="Total Points\n--")
            self.stats_avg_accuracy.configure(text="Avg Accuracy\n--")
            self.stats_avg_stars.configure(text="Avg Stars\n--")
            self.stats_last_updated_label.configure(text="Not paired")
            return

        # Update all stat labels
        self.stats_total_scores.configure(
            text=f"Total Scores\n{stats.get('total_scores', 0)}"
        )
        self.stats_records_held.configure(
            text=f"Records Held\n{stats.get('high_scores_held', 0)}"
        )
        self.stats_records_broken.configure(
            text=f"Records Broken\n{stats.get('record_breaks', 0)}"
        )
        self.stats_total_points.configure(
            text=f"Total Points\n{stats.get('total_points', 0):,}"
        )
        self.stats_avg_accuracy.configure(
            text=f"Avg Accuracy\n{stats.get('avg_accuracy', 0):.1f}%"
        )
        self.stats_avg_stars.configure(
            text=f"Avg Stars\n{stats.get('avg_stars', 0):.1f}"
        )

        # Update timestamp
        if self.stats_last_updated:
            time_ago = datetime.now() - self.stats_last_updated
            ...
            self.stats_last_updated_label.configure(text=f"Updated {time_str}")

    def fetch_user_stats(self) -> Optional[Dict]:
        """Fetch user stats from API"""
        if not self.discord_id or not self.auth_token:
            return None
        ...
        try:
            # Call bot API to get stats
            response = requests.get(
                f"{bot_url}/api/user/stats",
            ...
            if response.status_code == 200:
                ...
                self.log_activity(f"Stats fetch failed: HTTP {response.status_code}", "warning")
                return None

        except Exception as e:
            self.log_activity(f"Stats error: {e}", "warning")
            return None

    def refresh_user_stats(self):
        """Refresh user stats (called by button or automatically)"""
        if not self.discord_id:
            self.log_activity("Cannot fetch stats - not paired", "warning")
            return
        ...
        def fetch_in_background():
            ...
            if stats:
                ...
                self.after(0, lambda: self.update_user_stats_display(stats))
                self.after(0, lambda: self.log_activity("Stats refreshed", "success"))
    ...
    def initialize_tracker(self):
        ...
        self.discord_id = config.get('discord_id')
    ...
    def start_watcher(self):
        ...
        def on_new_score(score):
            ...
            def update_ui_with_score():
                ...
                self.log_activity(
                    f"Score: {final_song_data['song_title'][:30]} - {final_song_data['score']:,}",
                    "info"
                )
                self.after(15000, lambda: setattr(self, 'show_final_score', False))
                # Refresh stats after score submission
                self.after(2000, self.refresh_user_stats)
    ...
    def on_resync(self):
        ...
        if self.watcher:
            def do_resync():
                ...
                # Refresh stats after resync
                self.after(1000, self.refresh_user_stats)
